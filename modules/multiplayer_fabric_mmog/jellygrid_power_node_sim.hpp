...
/**************************************************************************/
/* Pure power-node simulation — no Godot, no api.hpp.                    */
...
static constexpr int   WINDOW_SAMPLES      = 64;

struct Sample { float t = 0.0f; float kw = 0.0f; };
...
inline void tick(State &s, float delta) {
	...
	if (s.overloaded) {
		...
		if (s.shutdown_timer <= 0.0f) { s.overloaded = false; s.shutdown_timer = 0.0f; }
		return;
	}
	s.current_kw -= s.current_kw * DECAY_RATE * delta;
	if (s.current_kw < 0.0f) s.current_kw = 0.0f;
}

inline void reset(State &s) { s = {}; }
